...
/*   Updated: 2020/10/13 18:00:00 by gdupont          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	not_builtin(char **cmd, t_env *envir)
{
	pid_t	pid_fork;
	int		pipe_c[3];
	char	*line;
...
void	call_command(t_list *args, char **cmd, int *status, t_env *envir)
{
...
//	close(STDERR_FILENO);
	...
	call_command(args, cmd, status, envir);
	free_args_list(args);
